...
def generate_svg_from_template(template_path: str, output_path: str, tokens: dict) -> bool:
    ...
    tokens_found = sum(1 for k in tokens if k in content)
    tokens_replaced = sum(1 for k in tokens if k not in content)
    print(f"  [+] Generated {os.path.basename(output_path)}  (replaced {tokens_replaced} tokens)")
    return True
